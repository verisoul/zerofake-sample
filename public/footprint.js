(function () {
    ...
    function printText(text) {
        ...
        should_print = should_print.toLowerCase() == "true";
        if (should_print) {
            console.log(text);
        }
    }
    function printConsoleError(error) {
        let should_print = "false";
        should_print = should_print.toLowerCase() == "true";
        if (should_print) {
            console.log(error);
    ...
    const createPerformanceLogger = () => {
        ...
        return {
            logTestResult: ({ test, passed, time = 0 }) => {
                ...
                const symbol = passed ? '✔' : '-';
                return printText(`%c${symbol}${time ? ` (${timeString})` : ''} ${test} ${result}`);
            },
            getLog: () => log,
            getTotal: () => total,
        };
...
`;
    ...
    if (!IS_WORKER_SCOPE) {
        ...
        printText(`${propsSearched.length} API properties analyzed in ${PROTO_BENCHMARK}ms (${lieList.length} corrupted)`);
    }
    ...
    async function spawnWorker() {
        ...
        const getWorkerData = async () => {
            const timer = createTimer();
            await queueEvent(timer);
            const userAgentData = await getUserAgentData(navigator).catch((error) => printConsoleError(error));
            // webgl
            ...
            const locationPathNameLie = (!href ||
                !pathname ||
                !/^\/(docs|footprintjs|public)|\/footprint.js$/.test(pathname) ||
    ...
    async function getBestWorkerScope() {
        try {
            ...
            const scriptSource = './footprint.js';
    ...
    const getPixelMods = () => {
        ...
        try {
            ...
            return printConsoleError(error);
        }
    };
    ...
    function getConsoleErrors() {
        try {
            ...
            logTestResult({ time: timer.stop(), test: 'printConsoleErrors', passed: true });
            return { errors };
        }
        catch (error) {
            logTestResult({ test: 'printConsoleErrors', passed: false });
            captureError(error);
            return;
        }
    }
    ...
    */
        ...
        try {
            ...
            const getPermissions = () => attempt(() => {
                ...
                const permissions = !('permissions' in navigator) ? undefined : Promise.all([
                    ...
                    acc[state] = [name];
                    return acc;
                }, {})).catch((error) => printConsoleError(error));
                return permissions;
            }, 'permissions failed');
        ...
        try {
            ...
            const getWebGLData = (gl, contextType) => {
                ...
                try {
                    ...
                    // printText([...pixels].filter(x => !!x)) // test read
                    return {
                        dataURI,
                        pixels: [...pixels],
                    };
    ...
    function filterJson(data) {
        // FP modifications 04_24_2023
        // canvas2d
        if (data.fp?.canvas2d) {
            delete data.fp.canvas2d?.emojiSet;
        }
        // canvasWebgl
        if (data.fp?.canvasWebgl) {
            delete data.fp.canvasWebgl?.extensions;
        }
        // clientRects
        if (data.fp?.clientRects) {
            delete data.fp.clientRects?.elementBoundingClientRect;
            delete data.fp.clientRects?.elementClientRects;
            delete data.fp.clientRects?.emojiSet;
            delete data.fp.clientRects?.rangeBoundingClientRect;
            delete data.fp.clientRects?.rangeClientRects;
        }
        // css
        if (data.fp?.css) {
            delete data.fp.css.computedStyle?.keys;
            delete data.fp.css.system?.colors;
            delete data.fp.css.system?.fonts;
        }
        // features
        if (data.fp?.features) {
            delete data.fp.features?.cssFeatures;
            delete data.fp.features?.cssVersion;
            delete data.fp.features?.jsFeatures;
            delete data.fp.features?.jsFeaturesKeys;
            delete data.fp.features?.jsVersion;
            delete data.fp.features?.version;
            delete data.fp.features?.versionRange;
            delete data.fp.features?.windowFeatures;
            delete data.fp.features?.windowVersion;
        }
        // fonts
        if (data.fp?.fonts) {
            delete data.fp.fonts?.apps;
            delete data.fp.fonts?.emojiSet;
            delete data.fp.fonts?.fontFaceLoadFonts;
            delete data.fp.fonts?.platformVersion;
        }
        // html element version
        if (data.fp?.htmlElementVersion) {
            delete data.fp.htmlElementVersion?.keys;
        }
        // maths
        if (data.fp?.maths) {
            delete data.fp.maths?.data;
        }
        // media
        if (data.fp?.media) {
            delete data.fp.media?.mimeTypes;
        }
        // navigator
        if (data.fp?.navigator) {
            delete data.fp.navigator?.plugins;
            delete data.fp.navigator?.properties;
        }
        // offlineAudioContext
        if (data.fp?.offlineAudioContext) {
            delete data.fp.offlineAudioContext?.binsSample;
            delete data.fp.offlineAudioContext?.copySample;
        }
        // svg
        if (data.fp?.svg) {
            delete data.fp.svg?.emojiSet;
        }
        // voices
        if (data.fp?.voices) {
            delete data.fp.voices?.languages;
            delete data.fp.voices?.local;
            delete data.fp.voices?.remote;
        }
        // windowFeatures
        if (data.fp?.windowFeatures) {
            delete data.fp.windowFeatures?.keys;
        }
        return data;
    }
    ...
    !async function () {
        ...
        let runServiceWorker = false;
        const serviceWorkerAttr = 'use-sw';
        const scriptElem = document.querySelector(`script[${serviceWorkerAttr}]`);
        if (scriptElem !== null) {
            const useSw = scriptElem.getAttribute(serviceWorkerAttr);
            if (useSw !== null && useSw.toLowerCase() === 'true') {
        ...
        const fingerprint = async () => {
            ...
            if (runServiceWorker) {
                promises.push(getBestWorkerScope());
            }
            else {
                promises.push(Promise.resolve());
            }
            // @ts-ignore
            const [voicesComputed, offlineAudioContextComputed, canvasWebglComputed, canvas2dComputed, windowFeaturesComputed, htmlElementVersionComputed, cssComputed, cssMediaComputed, screenComputed, mathsComputed, consoleErrorsComputed, timezoneComputed, clientRectsComputed, fontsComputed, mediaComputed, svgComputed, resistanceComputed, intlComputed, workerScopeComputed,] = await Promise.all(promises).catch((error) => printConsoleError$1(error.message));
            const navigatorComputed = await getNavigator(workerScopeComputed)
                .catch((error) => printConsoleError$1(error.message));
            // @ts-ignore
            ...
            // printText(`Fingerprinting complete in ${(fingerprintTimeEnd).toFixed(2)}ms`)
            // GPU Prediction
            ...
            // printText(performance.now()-start)
            const hashTimeEnd = hashStartTime();
            const timeEnd = timeStart();
            printText(`Hashing complete in ${(hashTimeEnd).toFixed(2)}ms`);
            if (PARENT_PHANTOM) {
                // @ts-ignore
            ...
            const fingerprint = {
                ...
                htmlElementVersion: !htmlElementVersionComputed ? undefined : {
                ...
                canvasWebgl: !canvasWebglComputed ? undefined : {
                    ...canvasWebglComputed,
                    pixels: pixelsHash,
                    pixels2: pixels2Hash,
                ...
                offlineAudioContext: !offlineAudioContextComputed ? undefined : {
        ...
        const { fingerprint: fp, styleSystemHash, styleHash, domRectHash, mimeTypesHash, canvas2dImageHash, canvas2dPaintHash, canvas2dTextHash, canvas2dEmojiHash, canvasWebglImageHash, canvasWebglParametersHash, deviceOfTimezoneHash, timeEnd, } = await fingerprint().catch((error) => printConsoleError$1(error)) || {};
        if (!fp) {
        ...
        const [fpHash, footprintHash] = await Promise.all([hashify(fp), hashify(footprint)])
            .catch((error) => {
            printConsoleError$1(error.message);
        }) || [];
        const data = {
            'fpHash': fpHash,
        ...
        const newJson = filterJson(data);
        if (window.fpCallback) {
            window.fpCallback(newJson, null);
        }
        else {
            printConsoleError$1('no callback');
        }
    }();
